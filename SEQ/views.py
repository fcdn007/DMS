...
def SequencingInfoV(request):
    ...
    if output_all_bool:
        serializer = SequencingInfoSerializer(SequencingInfo.objects.all(), many=True)
        col_name = ["上机文库号", "捕获文库名", "送测日期", "上机时间", "下机时间", "机器号", "芯片号", "备注", "上次修改时间",
                    "创建时间"]
        return output_model_all_records("SequencingInfo", serializer.data, col_name, drop_cols=['methycaptureinfo'])
    else:
...
def MethyQCInfoV(request):
    ...
    output_all_bool = request.GET.get('all', 0)
    if output_all_bool:
        serializer = MethyQCInfoSerializer(MethyQCInfo.objects.all(), many=True)
        col_name = ["索引", "Sample", "Data_Size(Gb)", "Clean_Rate(%)", "R1_Q20", "R2_Q20", "R1_Q30", "R2_Q30",
                    ...
                    "上次修改时间", "创建时间"]
        return output_model_all_records("MethyQCInfo", serializer.data, col_name,
